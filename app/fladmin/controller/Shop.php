...
class Shop extends Base
{
    ...
    public function index()
    {
        ...
        if (!empty($_REQUEST["keyword"])) {
            $where[] = array('company_name', 'like', '%' . $_REQUEST['keyword'] . '%');
        }
        if (!empty($_REQUEST["typeid"]) && $_REQUEST["typeid"] != 0) {
			$where[] = array('typeid', '=', $_REQUEST["typeid"]);
        }
        if (!empty($_REQUEST["id"])) {
			$where[] = array('typeid', '=', $_REQUEST["id"]);
        }
        $where[] = array('delete_time', '=', 0); //未删除
        if (!empty($_REQUEST["status"])) {
			$where[] = array('status', '=', $_REQUEST["status"]);
        }
        if (isset($_REQUEST["tuijian"])) {
			$where[] = array('tuijian', '=', $_REQUEST["tuijian"]);
        }
        if (isset($_REQUEST["category_id"])) {
			$where[] = array('category_id', '=', $_REQUEST["category_id"]);
        }

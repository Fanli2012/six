...
class Order extends Base
{
    ...
    {
        $res['count'] = self::where($where)->where('delete_time', self::ORDER_UNDELETE)->count();
        $res['list'] = array();

        if ($res['count'] > 0) {
            $res['list'] = self::where($where)->where('delete_time', self::ORDER_UNDELETE);
    ...
    {
        $res = self::where($where)->where('delete_time', self::ORDER_UNDELETE);
    ...
    {
        return self::where($where)->where('delete_time', self::ORDER_UNDELETE)->column($field);
    }
    ...
    public function getCountryNameAttr($value, $data)
    {
        if (isset($data['country_id']) && $data['country_id'] > 0) {
            return model('Region')->getFieldValue(array('id' => $data['country_id']), 'name');
        }
    ...
    public function getProvinceNameAttr($value, $data)
    {
        if (isset($data['province_id']) && $data['province_id'] > 0) {
            return model('Region')->getFieldValue(array('id' => $data['province_id']), 'name');
        }
    ...
    public function getCityNameAttr($value, $data)
    {
        if (isset($data['city_id']) && $data['city_id'] > 0) {
            return model('Region')->getFieldValue(array('id' => $data['city_id']), 'name');
        }
    ...
    public function getDistrictNameAttr($value, $data)
    {
        if (isset($data['district_id']) && $data['district_id'] > 0) {
            return model('Region')->getFieldValue(array('id' => $data['district_id']), 'name');
        }

...
class Goods extends Base
{
    ...
    {
        $res['count'] = self::where($where)->where('delete_time', self::GOODS_UNDELETE)->count();
        $res['list'] = array();

        if ($res['count'] > 0) {
            $res['list'] = self::where($where)->where('delete_time', self::GOODS_UNDELETE);
    ...
    {
        $res = self::where($where)->where('delete_time', self::GOODS_UNDELETE);
    ...
    {
        return self::where($where)->where('delete_time', self::GOODS_UNDELETE)->column($field);
    }
    ...
    {
        return model('GoodsType')->getFieldValue(array('id' => $data['type_id']), 'name');
    }

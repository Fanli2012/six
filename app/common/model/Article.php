...
class Article extends Base
{
    ...
    {
        $res['count'] = self::where($where)->where('delete_time', self::ARTICLE_UNDELETE)->count();
        $res['list'] = array();

        if ($res['count'] > 0) {
            $res['list'] = self::where($where)->where('delete_time', self::ARTICLE_UNDELETE);
    ...
    {
        $res = self::where($where)->where('delete_time', self::ARTICLE_UNDELETE);
    ...
    {
        return self::where($where)->where('delete_time', self::ARTICLE_UNDELETE)->column($field);
    }
    ...
    {
        return model('ArticleType')->getFieldValue(array('id' => $data['type_id']), 'name');
    }

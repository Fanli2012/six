...
use app\common\validate\UserBonus as UserBonusValidate;
...
class UserBonusLogic extends BaseLogic
{
    ...
    {
        return validate(UserBonusValidate::class);
    }
    ...
    public function getAvailableBonusList($data)
    {
		...
		$where[] = array('status', '=', UserBonus::USER_BONUS_STATUS_UNUSED);
		$where[] = array('end_time', '>=', time()); //有效期

        //满多少使用
        if (isset($data['min_amount'])) {
            $where[] = array('min_amount', '<=', $data['min_amount']);
            $where[] = array('bonus_money', '<=', $data['min_amount']);
        }

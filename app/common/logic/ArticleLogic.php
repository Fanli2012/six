...
class ArticleLogic extends BaseLogic
{
    ...
    {
        return validate(ArticleValidate::class);
    }
    ...
    public function getList($where = array(), $order = '', $field = '*', $offset = 0, $limit = 15)
    {
        ...
        if ($res['count'] > 0) {
            foreach ($res['list'] as $k => $v) {
                $res['list'][$k] = $res['list'][$k]->withAttr('type_name_text', function ($value, $data) {
                    return model('ArticleType')->getFieldValue(array('id' => $data['type_id']), 'name');
                })->withAttr('status_text', function ($value, $data) {
                    $article_status_desc = Article::$article_status_desc;
                    return $article_status_desc[$data['status']];
                });
            }
        }
    ...
    public function getOne($where = array(), $field = '*')
    {
        ...
        $this->getModel()->getDb()->where($where)->inc('click', 1)->update();
    ...
    public function replaceKeyword($body)
    {
        ...
        if (!$posts) {
            $posts = db("keyword")->select()->toArray();

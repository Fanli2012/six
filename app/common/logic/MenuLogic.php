...
class MenuLogic extends BaseLogic
{
    ...
    {
        return validate(MenuValidate::class);
    }
    ...
    public function edit($data, $where = array())
    {
        ...
        //判断菜单名称
        if (isset($data['name']) && !empty($data['name'])) {
            $where_name[] = array('name', '=', $data['name']);
            $where_name[] = array('id', '<>', $record['id']); //排除自身
        ...
        //判断模型-控制器-方法
        $where_module_controller_action[] = array('module', '=', $data['name']);
        $where_module_controller_action[] = array('controller', '=', $data['name']);
        $where_module_controller_action[] = array('action', '=', $data['name']);
        $where_module_controller_action[] = array('id', '<>', $record['id']); //排除自身
        if ($this->getModel()->getOne($where_module_controller_action)) {
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, '该模型-控制器-方法已存在');
        }

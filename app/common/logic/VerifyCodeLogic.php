...
use app\common\validate\VerifyCode as VerifyCodeValidate;
...
class VerifyCodeLogic extends BaseLogic
{
    ...
    {
        return validate(VerifyCodeValidate::class);
    }
    ...
    public function getVerifyCodeBySmsbao($data)
    {
		...
		try {
            $this->getValidate()->scene('get_smscode_by_smsbao')->check($data);
        } catch (ValidateException $e) {
            // 验证失败 输出错误信息
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
        }
    ...
    public function check($data)
    {
		...
		try {
            $this->getValidate()->scene('check')->check($data);
        } catch (ValidateException $e) {
            // 验证失败 输出错误信息
            return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
        }

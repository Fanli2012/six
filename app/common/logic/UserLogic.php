...
class UserLogic extends BaseLogic
{
    ...
    {
        return validate(UserValidate::class);
    }
    ...
    public function login($data)
    {
        ...
        $map1 = [
            ['user_name', '=', $user_name],
            ['password', '=', $password],
            ['delete_time', '=', User::USER_UNDELETE],
        ];
        $map2 = [
            ['email', '=', $user_name],
            ['password', '=', $password],
        ...
        $user = $this->getModel()->getDb()->whereOr([$map1, $map2, $map3])->find();
        if (!$user) {
    ...
    public function wxLogin($data)
    {
        ...
        if (!$user) {
			...
			try {
				$this->getValidate()->scene('wx_register')->check($data);
			} catch (ValidateException $e) {
				// 验证失败 输出错误信息
				return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
			}
    ...
    public function register($data)
    {
		...
		try {
			$this->getValidate()->scene('register')->check($data);
		} catch (ValidateException $e) {
			// 验证失败 输出错误信息
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
		}
    ...
    public function userInfoUpdate($data, $where = array())
    {
		...
		$validate = Validate::rule([
            ...
            'email' => 'email',
            'nickname' => 'max:30',
            'user_name' => 'max:30|regex:/^[-_a-zA-Z0-9]{2,18}$/i',
            'head_img' => 'max:250',
            'sex' => 'in:0,1,2',
			...
			'user_name.regex' => '用户名2-18个字符，包含数字，大小写字母',
    ...
    public function userPasswordUpdate($data, $where = array())
    {
		...
		try {
			$this->getValidate()->scene('user_password_update')->check($data);
		} catch (ValidateException $e) {
			// 验证失败 输出错误信息
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
		}
    ...
    public function userPayPasswordUpdate($data, $where = array())
    {
		...
		try {
			$this->getValidate()->scene('user_pay_password_update')->check($data);
		} catch (ValidateException $e) {
			// 验证失败 输出错误信息
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
		}
    ...
    public function pcRegister($data)
    {
		...
		try {
			$this->getValidate()->scene('pc_mobile_reg')->check($data);
		} catch (ValidateException $e) {
			// 验证失败 输出错误信息
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
		}
    ...
    public function pcResetPwd($data)
    {
		...
		try {
			$this->getValidate()->scene('pc_resetpwd')->check($data);
		} catch (ValidateException $e) {
			// 验证失败 输出错误信息
			return ReturnData::create(ReturnData::PARAMS_ERROR, null, $e->getError());
		}

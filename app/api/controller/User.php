...
class User extends Base
{
    ...
    public function myteam()
    {
        ...
        $where = array();
		$where[] = array('parent_id', '=', $this->login_info['id']);
        if (input('sex', '') !== '') {
			$where[] = array('sex', '=', input('sex'));
        }
        if (input('group_id', '') !== '') {
			$where[] = array('group_id', '=', input('group_id'));
        }
        if (input('status', '') === '') {
			$where[] = array('status', '=', UserModel::USER_STATUS_NORMAL);
        } else {
            if (input('status') != -1) {
    ...
    public function user_info_update()
    {
        if (Helper::isPostRequest()) {
            ...
            if (input('refund_account', '') !== '') {
                $data['refund_account'] = input('refund_account');
            }
            if (input('refund_name', '') !== '') {
                $data['refund_name'] = input('refund_name');
            }
    ...
    public function bind_wechat_miniprogram_user_mobile()
    {
        ...
        // 判断手机号是否存在
        $user = model('User')->getOne(array(array('mobile', '=', $res['data']['purePhoneNumber']), array('id', '<>', $this->login_info['id'])));
        if ($user) {

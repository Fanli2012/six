...
class Slide extends Common
{
    ...
    public function index()
    {
        ...
        if (input('target', '') != '') {
			$where[] = array('target', '=', input('target'));
        }
        if (input('group_id', '') > 0) {
			$where[] = array('group_id', '=', input('group_id'));
        }
        if (input('status', '') === '') {
			$where[] = array('status', '=', SlideModel::SLIDE_STATUS_NORMAL);
        } else {
            if (input('status') != -1) {
        ...
        if ($res['count'] > 0) {
            foreach ($res['list'] as $k => $v) {
                if ($v['pic']) {
                    $res['list'][$k]['pic'] = get_site_cdn_address() . $v['pic'];
                }
            }
        }
    ...
    public function detail()
    {
        ...
        if ($res['pic']) {
            $res['pic'] = get_site_cdn_address() . $res['pic'];
        }

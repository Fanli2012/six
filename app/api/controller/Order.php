...
class Order extends Base
{
    ...
    public function index()
    {
        ...
        if ($status !== '' && $status != 0) {
            if ($status == 1) {
				...
				$where[] = array('order_status', '=', 0);
				$where[] = array('shipping_status', '=', 0);
				$where[] = array('pay_status', '=', 1);
            } elseif ($status == 3) {
				$where[] = array('order_status', '=', 0);
				$where[] = array('shipping_status', '=', 1);
				$where[] = array('pay_status', '=', 1);
				$where[] = array('refund_status', '=', 0);
            } elseif ($status == 4) {
				$where[] = array('order_status', '=', 3);
				$where[] = array('shipping_status', '=', 2);
				$where[] = array('is_comment', '=', 0);
				$where[] = array('refund_status', '=', 0);
            } elseif ($status == 5) {
				$where[] = array('order_status', '=', 3);
				$where[] = array('refund_status', '<>', 3);
            }
        }
            ...
            foreach ($res['list'] as $k => $v) {
                if ($res['list'][$k]['goods_list']) {
                    ...
                    foreach ($goods_list as $key => $value) {
                        if ($value['goods_img']) {
                            $goods_list[$key]['goods_img'] = get_site_cdn_address() . $value['goods_img'];
                        }
                    }
    ...
    public function detail()
    {
        ...
        if (!$res) {
            Util::echo_json(ReturnData::create(ReturnData::RECORD_NOT_EXIST));
        }

        if ($res['goods_list']) {
            foreach ($res['goods_list'] as $k => $v) {
                if ($v['goods_img']) {
                    $res['goods_list'][$k]['goods_img'] = get_site_cdn_address() . $v['goods_img'];
                }
            }
        }
    ...
    public function wxminiprogram_pay()
    {
        ...
        if (!($order_info['order_status'] == 0 && $order_info['pay_status'] == 0)) {
            Util::echo_json(ReturnData::create(ReturnData::RECORD_NOT_EXIST, null, '订单已过期'));
        }

        //微信支付-start
        require_once root_path() . 'extend/wxpay/WxPayMiniprogramConfig.php'; // 导入微信配置类
        require_once root_path() . 'extend/wxpay/WxPayPubHelper.class.php'; // 导入微信支付类
        ...
        if ($jsApiParameters) {
            $return_data['jsApiParameters'] = $jsApiParameters;
        }

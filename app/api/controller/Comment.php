...
class Comment extends Base
{
    ...
    public function index()
    {
        ...
        if (input('comment_type', '') != '') {
			$where[] = array('comment_type', '=', input('comment_type'));
        }; //0商品评价，1文章评价
        if (input('comment_rank', '') != '') {
			$where[] = array('comment_rank', '=', input('comment_rank'));
        }
        if (input('id_value', '') != '') {
			$where[] = array('id_value', '=', input('id_value'));
        }
        if (input('parent_id', '') != '') {
			$where[] = array('parent_id', '=', input('parent_id'));
        }
		$where[] = array('status', '=', Comment::SHOW_COMMENT);
		$where[] = array('user_id', '=', $this->login_info['id']);
    ...
    {
        if (input('comment', '') == '' || input('order_id', '') == '') {
            Util::echo_json(ReturnData::create(ReturnData::PARAMS_ERROR));
        }
